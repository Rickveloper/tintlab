...
/* ========= Perf: pixel ratio scaler + visibility + context ========= */

const DPR_MIN = 0.7;
const DPR_MAX = 1.25;
let dynamicDPR = Math.min(window.devicePixelRatio || 1, 1.0); // start conservative
let lastFrame = performance.now();
let ema = 16; // ~60fps
const EMA_WEIGHT = 0.15;

function adjustResolution() {
  // simple EMA of frame time to determine up/down
  const now = performance.now();
  const dt = now - lastFrame;
  lastFrame = now;
  ema = (1-EMA_WEIGHT)*ema + EMA_WEIGHT*dt;

  // targets: ~16.6ms (60fps). Allow 14-20ms band.
...
function initRenderer(){
  ...
  // controls will trigger renders while moving
  // We attach after camera is created in initScene()
}
...
function initScene(){
  ...
  controls.addEventListener('change', () => {
    adjustResolution(); // as the user moves the camera, adapt res
    invalidate();
  });
...
function createPlaceholderCar(){
  ...
  windshield.name = 'windshield'; windshield.rotation.x = -Math.PI/9; windshield.position.set(0.2, 1.3, 0.86);
...
function applyAllTints(){
  ...
  if(state.uniform){
    ...
    glassMeshes.forEach(m => {
      const v = (state.windows[m.name] ?? state.shade)/100;
      applyTintToMesh(m, v, film);
    });
  }
  invalidate();
}
...
function select(mesh){
  ...
  selected.add(mesh); addOutline(mesh);
}
function deselect(mesh){
  if(!selected.has(mesh)) return;
  selected.delete(mesh); removeOutline(mesh);
}
function clearSelection(){
  [...selected].forEach(removeOutline);
  selected.clear(); updateSelectedLabel();
}
...
function bindUI(){
  ...
  filmSel.value = state.film; shadeSel.value = String(state.shade);
...
function writeURL(){
  ...
  const winStr = [
    ...
    `lf:${state.windows.lf}`, `rf:${state.windows.rf}`,
    `lr:${state.windows.lr}`, `rr:${state.windows.rr}`,
...
function restoreFromURL(){
  ...
  const w=q.get('w'); if(w){
    w.split(',').forEach(tok=>{
      const [code,valStr]=tok.split(':'); const key=CODE_TO_KEY[code];
